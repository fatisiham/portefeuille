    ...
    <div class="art-block-body">
      ...
      <div class="left_sidebar">
        ...
        <div id="left_menu">
			...
			<li><a href="ajout projet.php">Gestion de phases</a></li>
...
<label>Nom sous famille</label><select id="user" name="famille">
<option value="Vide">Veuillez selectionner une famille</option>
<?php
require_once("includes/cnx.php");
$requete = $cnx->query("SELECT code_ss_famille , nom_ss_famille FROM ss_famille");
...
if (isset ($_POST["famille"]))
   { require_once("includes/cnx.php");
	$requete = $cnx->prepare("SELECT * FROM projet WHERE code_ss_famille LIKE ?");
	$requete->execute(array($_POST["famille"]));
	$rech = $cnx->prepare("SELECT nom , prenom FROM intervenant WHERE code_intervenant LIKE ?");
	...
	$out = '<style type="text/css">
			...
			$out .="liste des projet appartenants a cette sous famille : </br></br>"; 
	while($data = $requete->fetch() )
	   { 	$rech = $cnx->prepare("SELECT nom , prenom FROM intervenant WHERE code_intervenant LIKE ?");
            $rech->execute(array($data["code_cp"]));
	        $chef=$rech->fetch();

	
		$out .= "Nom du projet :<span class='info'>".$data["nom_p"]."</span>";
	    $out .= "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Nom chef de projet:<span class='info'>".$chef["nom"]."&nbsp".$chef["prenom"]."</span><br/>";}

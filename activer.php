...
{ if (isset ( $_POST["id"] )){
	...
	$requete = $cnx->exec("UPDATE intervenant SET statut=1 WHERE code_intervenant=".$_POST["id"]);
	if( $requete > 0 )

	{$Erreur="true";
	 $out='Le compte a été activé !';
	 }
	 else
	 {$Erreur="true";
	 $out='Erreur !';}
...
<div class="art-nav-inner">
	<ul class="art-hmenu">
			...
			<li>
			<a href="./aj uti.php" class="active"><span class="l"></span><span class="r"></span><span class="t">Gestion des utilisateurs</span></a>
		</li>
...
<label>Liste des comptes d&eacute;sactiv&eacute;s : </label><select id="user" name="id">
<option value="Vide">Veuillez choisir un utilisateur</option>
<?php
require_once("includes/cnx.php");
$requete = $cnx->query("SELECT code_intervenant ID,login,CONCAT(nom,' ',prenom) AS user FROM intervenant where statut=2");
while($data = $requete->fetch() )
	echo '<option value='.$data["ID"].'>'.$data["login"].' : '.$data["user"].'</option>';
?>
</select><br/>
<div id="buttons">
<input type="submit" value="Activer" id="valider" name="valider" /> <br/>
</div>
</form></p>
                </div>
                <div class="cleared"></div>

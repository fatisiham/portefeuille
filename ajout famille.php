...
//NOM
	...
	{
		$erreur = true;
		$StringErreur = "Veuillez remplir le champs : nom famille !";
	}
	if( $erreur == false )
	{
		require_once("includes/cnx.php");
		$requete = $cnx->prepare("SELECT * FROM famille WHERE nomfamille LIKE ?");
		$requete->execute(array($_POST["nom"]));
		if($requete->rowCount() > 0)
		{
			$erreur = true;
			$StringErreur = "famille existe déjà !";
			goto SortieSi;
		}
	 	$insertion = $cnx->prepare("INSERT INTO famille(nomfamille,date_creation) VALUES(?,NOW())");
		$insertion->execute(array($_POST["nom"]));
		if( $insertion->rowCount() > 0 )
		{
			$erreur = true;
			$StringErreur = 'famille ajout\351e avec succ\351s !'; 
...
<div class="art-nav-inner">
	<ul class="art-hmenu">
			...
			<li>
			<a href="#" class="active"><span class="l"></span><span class="r"></span><span class="t">Gestion de projets</span></a>
		</li>	
...
<input type="submit" name="ajout" value="Cr&eacute;er famille"  /><br/>
</form>
